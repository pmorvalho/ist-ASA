...
void criaGrafo(){
    ...
    g = vector< vector< Aresta> >(N+1);
    filiais.resize(F);
    arestas=vector<Aresta>(C);
...
int johnson() {
  ...
  vector<int> valores_bellmanford = bellmanford(0);
  ...
  for (unsigned int  i=0; i < filiais.size(); i++) {
    caminho_filial = dijkstra(filiais[i]);
  ...
  if(local_encontro==-1) { /* quando nao ha um ponto de encontro possivel (vertice isolado) */
    ...
    for (unsigned int i=0; i < filiais.size(); i++) {
     vector<int> distancias = dijkstra(filiais[i]);
     distancias[local_encontro] += valores_bellmanford[local_encontro] - valores_bellmanford[filiais[i]];
