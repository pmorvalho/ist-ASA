   ...
   vector<int> alturas(g.size(), 0);
      ...
      for (unsigned int  k = 1; k < g.size(); k++) {
        ...
        for (unsigned int  j=0; j < arestas.size(); j++) {
          int t=j;
          Aresta a = arestas[t];
          /* Faz a relaxacao das arestas do grafo */
            if (alturas[a.origem] + a.custo < alturas[a.destino]) {
              alturas[a.destino] = alturas[a.origem] + a.custo;
            }
        }
